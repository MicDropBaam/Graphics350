...
Rendering::Rendering()
	...
	rotation(0.f), width(512), height(512), isUseTex_(false), isUseBV_(false), pca_mid(0)
	, x_minpt(0), x_maxpt(0), y_maxpt(0), y_minpt(0), z_minpt(0), z_maxpt(0),
	isUseRotation_(false)
{
}
...
void Rendering::DivideColor(Camera* camera, Shader* shader, float aspect, glm::vec3 m_position)
{
...
void Rendering::BspColor(Camera * camera, Shader * shader, float aspect, glm::vec3 m_position)
{
	const static glm::vec3 up(0, 1, 0);
...
void Rendering::Render(Camera* camera, Shader* shader, float aspect, glm::vec3 m_position, bool isUseBV, bool isUseRotation)
{
	UNREFERENCED_PARAMETER(isUseRotation);
	const static glm::vec3 up(0, 1, 0);
...
void Rendering::Draw(std::vector<glm::vec3>& vert, std::vector<unsigned>& indices, std::vector<glm::vec2> textures)
{
...
void Rendering::DrawOctree(std::vector<glm::vec3>& vert, std::vector<unsigned>& indices, std::vector<glm::vec2> textures)
{
...
void Rendering::DrawBsptree(std::vector<glm::vec3>& vert, std::vector<unsigned>& indices, std::vector<glm::vec2> textures)
{
...
bool Rendering::LoadObj(const char * path, bool isUseTex, bool isUseBV, glm::vec3 midpoint)
{
	UNREFERENCED_PARAMETER(isUseBV);
	glm::vec3 P1, P2;
...
glm::vec3 Rendering::GetCentroid()
{
...
float Rendering::GetRitter()
{
	float xmax_ = 0, xmin_ = 0, ymax_ = 0, ymin_ = 0, zmax_ = 0, zmin_ = 0;
	glm::vec3 xmaxVert(0), xminVert(0), ymaxVert(0), yminVert(0), zmaxVert(0), zminVert(0), minVertices(0), maxVertices(0);
	for (unsigned int i = 0; i < out_vertices.size(); i++)
	{
...
glm::mat4 Rendering::SetRotation(glm::mat3& mat)
{
	glm::mat4 res;
...
float Rendering::Variance(float x[], int n)
{
	float u = 0.0f;
	for (int i = 0; i < n; i++)
		u += x[i];
...
void Rendering::CovarianceMatrix(glm::mat3x3 &cov, std::vector<glm::vec3>& vert, int numPts)
{
...
void Rendering::SymSchur2(glm::mat3x3 &a, int p, int q, float &c, float &s)
{
...
void Rendering::Jacobi(glm::mat3x3 &a, glm::mat3x3 &v)
{
	int i, j, n, p, q;
	float prevoff=0, c, s;
	glm::mat3x3 J;
...
void Rendering::ExtremePointsAlongDirection(glm::vec3& dir, std::vector<glm::vec3>& vert, int n, int *imin, int *imax)
{
...
bool Rendering::loadPPM(const char* path, std::vector<glm::vec3>& values_)
{
	FILE* file2;
...
void Rendering::TextureData(Shader* shader)
{
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, tex1);
