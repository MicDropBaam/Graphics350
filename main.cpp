...
enum BV_Methods { DEFAULT, AABB, CENTROID, RITTER, LARSSON, 
				BS_PCA, ELLIPSE_PCA, OBB_PCA, 
				ROOT, DEPTH1, DEPTH2, DEPTH3 };
BV_Methods BV_methods;
...
Camera cam(glm::vec3(0.0f, 1.5f, 15.0f));
...
bool DetectCollision_BroadPhase(Rendering* Sphere, Rendering* mod);
...
int main()
{
	...
	//Create Window
	GLFWwindow* window = glfwCreateWindow(Width, Height, "CS350_SeraphinaKim_3", NULL, NULL);
	if (window == NULL)
	{
	...
	struct ModelData
	{
		const char* model_name[6] = { "models\\sphere_mid_poly.obj", "models\\sphere_mid_poly.obj", 
										"models\\sphere_mid_poly.obj","models\\sphere_mid_poly.obj",
										"models\\sphere_mid_poly.obj", "models\\sphere_mid_poly.obj" };


		glm::vec3 modelpos[6] = { glm::vec3(-3.5f, -1.f, -3.0f), glm::vec3(5.0f, 1.5f, -3.5f),
								glm::vec3(-0.5f, -2.f, 3.0f), glm::vec3(2.5f, -1.f, 4.f),
								glm::vec3(-1.5f, 2.f, 0.f), glm::vec3(1.5f,3.f,1.f) };
	...
	struct BV
	{
		const char* bv_sphere = "models\\sphere_mid_poly.obj";
		const char* bv_box = "models\\cube_low_poly.obj";
		float ritter_rad;
	};
	BV bv_data;
	...
	Rendering positon_model;
	Rendering Object[6];
	std::vector<Rendering*> model_vec;
	...
	Rendering shooting;
	shooting.position = cam.position;
	shooting.color = glm::vec3(1.0f, 1.0f, 0.f);
	shooting.LoadObj("models\\sphere_mid_poly.obj", false, false, shooting.position);
	shooting.Draw(shooting.out_vertices, shooting.vertex_indices, shooting.textureUV);
	shooting.scale = glm::vec3(0.0f, 0.0f, 0.0f);
	shooting.TextureProcess();

	Node *node;
	bvh_tree.TopDownBV(&node, vertex_chunk, tree_depth);
	...
	OctreeNode* oc_node = Octree(model_vec, GetCenter(model_vec), GetLongest(model_vec), 8);
	BSPNode* bsp_node = BSPTree(world_triangle, model_vec, 8, obj_index, obj_indices);

	Rendering Octree;
	Octree.LoadObj(bv_data.bv_box, false, true, oc_node->oc_center);
	Octree.Draw(Octree.out_vertices, Octree.vertex_indices, Octree.textureUV);

	Rendering FillBsptree[6];
	...
	for (int i = 0; i < 6; i++)
	{
		...
		AABB[i].color = glm::vec3(1.f, 0.1f, 0.1f);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		Ritter[i].color = glm::vec3(1.f, 1.f, 0.5f);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		EOPS[i].color = glm::vec3(0.3f, 0.7f, 0.2f);
	...
	//Navy
	Rendering Ellipsoid[6];
	for (int i = 0; i < 6; i++)
	{
		Ellipsoid[i].position = model_data.modelpos[i];
		Ellipsoid[i].color = glm::vec3(0.2f, 0.4f, 1.f);
	...
	for (int i = 0; i < 6; i++)
	{
		...
		OBB[i].color = glm::vec3(0.6f, 0.2f, 1.f);
	...
	//light spheres
	Rendering Object2[16];
	Object2[0].LoadObj("models\\sphere_high_poly.obj", false, false, glm::vec3(0, 0, 0));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (show_demo_window)
		{
			...
			if (ImGui::SliderInt("SPHERES", &directionalLight.sphere_max, 0, 16)) {}
			ImGui::Checkbox("ROTATION", &sphere_data.sphere_rotating);
			...
			if (ImGui::CollapsingHeader("Spatial Partitioning"))
			{
				if (ImGui::Button("Draw Octree"))
					isOctree = true;
				ImGui::SameLine();
				if (ImGui::Button("ShutDown Octree"))
					isOctree = false;
				if (ImGui::Button("Draw Bsptree"))
					isBsptree = true;
				ImGui::SameLine();
				if (ImGui::Button("ShutDown Bsptree"))
					isBsptree = false;
			}
			ImGui::End();

			ImGui::Begin("SECOND");
			ImGui::Text("Controlling Depth");

			if (ImGui::Button("Show BVH"))
			{
				isbvh = true;
			}
			if (ImGui::Button("Hide BVH"))
			{
				isbvh = false;
			}
			ImGui::SliderInt("Top Down", &tree_depth, 1, 7, "%d");
			if (ImGui::Button("BOX"))
			{
				bvh_drawType = 0;
			}
			...
			if (ImGui::Button("DEFAULT"))
				BV_methods = BV_Methods::AABB;
			if (ImGui::Button("AABB"))
				BV_methods = BV_Methods::AABB;
			if (ImGui::Button("CENTROID"))
				BV_methods = BV_Methods::CENTROID;
			if (ImGui::Button("RITTER"))
				BV_methods = BV_Methods::RITTER;
			if (ImGui::Button("LARSSON"))
				BV_methods = BV_Methods::LARSSON;
			if (ImGui::Button("BS_PCA"))
				BV_methods = BV_Methods::BS_PCA;
			if (ImGui::Button("ELLIPSE_PCA"))
				BV_methods = BV_Methods::ELLIPSE_PCA;
			if (ImGui::Button("OBB_PCA"))
				BV_methods = BV_Methods::OBB_PCA;
		...
		if (isOctree == true)
		{
			...
			{
				FillOctree[i].octree_color = Object[i].octree_color;
				FillOctree[i].DivideColor(&cam, &shader_octree, aspect, FillOctree[i].position);
			}
		}
		...
		if (isBsptree == true)
		{
			...
			{
				FillBsptree[i].bsptree_color = Object[i].bsptree_color;
				FillBsptree[i].BspColor(&cam, &shader_octree, aspect, FillBsptree[i].position);
			}
		}
		...
		shooting.Render(&cam, &shader, aspect, shooting.position, false, false);
		//shooting spheres
		glm::vec3 dir_vec = glm::normalize(cam.front);
		if (isShooting == true)
		{
			shooting.scale = glm::vec3(1.0f, 1.0f, 1.0f);
			for (unsigned int i = 0; i < 6; i++)
			{
				if (DetectCollision_BroadPhase(&shooting, &Object[i]))
				{
					shooting.color = glm::vec3(1.0f, 0.f, 0.f);
					dir_vec = glm::vec3(0.f, 0.f, 0.f);
					isCollide = true;
				...
				else
				{
					if (isCollide == true)
					{
						...
						shooting.position += dir_vec * 0.01f;
				}
			}
		}
		else
		{
			...
			isCollide = false;
			shooting.color = glm::vec3(1.0f, 1.0f, 0.f);
			shooting.scale = glm::vec3(0.f, 0.f, 0.f);
		}
...
bool DetectCollision_BroadPhase(Rendering* Sphere, Rendering* mod)
{
	for (unsigned int i = 0; i < 6; i++)
	{
		if (glm::abs(Sphere->position.x - mod->position.x) >= glm::abs(Sphere->aabb().x + mod->aabb().x))
			return false;
		if (glm::abs(Sphere->position.y - mod->position.y) >= glm::abs(Sphere->aabb().y + mod->aabb().y))
			return false;
		if (glm::abs(Sphere->position.z - mod->position.z) >= glm::abs(Sphere->aabb().z + mod->aabb().z))
			return false;
